...
import updateStreak from 'utils/updateStreak';\n`;
    ...
    code = code +
      `export const ${gameTitle.replace(' ', '')} = (req: Request, res: Response, next: NextFunction) => {
...
try {
  factory().then(async (instance) => {
    ...
    for (let i = 0; i < export_vars.length; i++) {
      ...
      if (cur_var.length === 0)
        continue;
      code = code + `    response['${cur_var}'] = ${cur_var};\n`;
    }
    code = code +
      `    res.customSuccess(200, 'Instance Generated', response);
  });
} catch (err) {
...
import updateStreak from 'utils/updateStreak;\n`;
    ...
    let code =
      `export const ${gameTitle.replace(' ', '')}Validate = (req: Request, res: Response, next: NextFunction) => {

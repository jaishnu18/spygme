...
export function* getCrossword() {
  try {
    const response = yield api.get(`/game/arc-consistency/graded-quiz`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/arc-consistency/graded-quiz/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export function* saveFeedback(action) {
  try {
    ...
    const response = yield api.put(
      `/game/arc-consistency/feedback-save`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* gradedArcConsistencySaga() {

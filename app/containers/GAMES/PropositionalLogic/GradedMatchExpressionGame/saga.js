...
export function* getExpression() {
  try {
    const response = yield api.get(`/game/match-expression/graded-quiz`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/match-expression/graded-quiz/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* gradedMatchExpressionGameSaga() {
  yield all([

...
export function* getEntropyBucket(action) {
  try {
    const level = action.payload;

    const response = yield api.get(`/game/entropy/question/${level}`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/entropy/question/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* entropyCalculationSaga() {
  // See example in containers/HomePage/saga.js
  yield all([
    takeLatest(GET_GAME_DATA_START, getEntropyBucket),

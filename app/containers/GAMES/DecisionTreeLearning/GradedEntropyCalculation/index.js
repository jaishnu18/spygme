...
import GameComponent from 'components/GAMES/DecisionTreeLearning/GradedEntropyCalculationComponent';
import makeSelectGradedEntropyCalculation from './selectors';
...
export function GradedEntropyCalculation(props) {
  useInjectReducer({ key: 'gradedEntropyCalculation', reducer });
  useInjectSaga({ key: 'gradedEntropyCalculation', saga });

  const maxLevel = 3;

  const [currentLevel, setCurrentLevel] = useState(-1);
  const [movement, setMovement] = useState([]);
  const [value, setValue] = useState(Array(maxLevel).fill(undefined));
  ...
  return (
    <div>
      <Helmet>
        <title>GradedEntropyCalculation</title>
        <meta
          name="description"
          content="Description of GradedEntropyCalculation"
        />
      </Helmet>
...
GradedEntropyCalculation.propTypes = {
...
const mapStateToProps = createStructuredSelector({
  state: makeSelectGradedEntropyCalculation(),
});

...
export function* getData() {
  try {
    const response = yield api.get(`/game/find-feature-with-highest-ig/graded-quiz`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/find-feature-with-highest-ig/graded-quiz/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export function* saveFeedback(action) {
  try {
    ...
    const response = yield api.put(
      `/game/find-feature-with-highest-ig/feedback-save`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* gradedFindFeatureWithHighestIgSaga() {

/**
 *
 * GradedFindFeatureWithHighestIg
 *
 */
...
import GameComponent from 'components/GAMES/DecisionTreeLearning/GradedFindFeatureWithHighestIgComponent';
import makeSelectGradedFindFeatureWithHighestIg from './selectors';
...
export function GradedFindFeatureWithHighestIg(props) {
  useInjectReducer({ key: 'gradedFindFeatureWithHighestIg', reducer });
  useInjectSaga({ key: 'gradedFindFeatureWithHighestIg', saga });
  ...
  const submit = () => {
    ...
    for (let i = 0; i < gameData.length; i += 1) {
      gameData[i].response = {
        ...
        answer: Object.keys(value[i]).length === 0 ? undefined : value[i],
      };
    }
  ...
  return (
    <div>
      <Helmet>
        <title>GradedFindFeatureWithHighestIg</title>
        <meta
          name="description"
          content="Description of GradedFindFeatureWithHighestIg"
        />
      </Helmet>
...
GradedFindFeatureWithHighestIg.propTypes = {
...
const mapStateToProps = createStructuredSelector({
  state: makeSelectGradedFindFeatureWithHighestIg(),
});

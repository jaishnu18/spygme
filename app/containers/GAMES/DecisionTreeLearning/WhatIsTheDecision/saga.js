...
export function* getDataset(action) {
  try {
    ...
    const response = yield api.get(`/game/what-is-the-decision/question/${level}`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/what-is-the-decision/question/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* whatIsTheDecisionSaga() {

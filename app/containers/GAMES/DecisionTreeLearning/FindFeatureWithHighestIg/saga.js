...
export function* getDataset(action) {
  try {
    ...
    const response = yield api.get(`/game/find-feature-with-highest-ig/question/${level}`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/find-feature-with-highest-ig/question/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* findFeatureWithHighestIgSaga() {

...
export function* getData() {
  try {
    const response = yield api.get(`/game/what-is-the-decision/graded-quiz`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/what-is-the-decision/graded-quiz/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export function* saveFeedback(action) {
  try {
    ...
    const response = yield api.put(
      `/game/what-is-the-decision/feedback-save`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* gradedWhatIsTheDecisionSaga() {

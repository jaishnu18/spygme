...
export function* getData() {
  try {
    const response = yield api.get(`/game/evaluate-ig-expression/graded-quiz`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/evaluate-ig-expression/graded-quiz/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export function* saveFeedback(action) {
  try {
    ...
    const response = yield api.put(
      `/game/evaluate-ig-expression/feedback-save`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* gradedEvaluateIgExpressionSaga() {

...
import GameComponent from 'components/GAMES/DecisionTreeLearning/GradedEvaluateIgExpressionComponent';
import makeSelectGradedEvaluateIgExpression from './selectors';
...
export function GradedEvaluateIgExpression(props) {
  useInjectReducer({ key: 'gradedEvaluateIgExpression', reducer });
  useInjectSaga({ key: 'gradedEvaluateIgExpression', saga });
  ...
  const submit = () => {
    ...
    for (let i = 0; i < gameData.length; i += 1) {
      gameData[i].response = {
        ...
        feature1: gameData[i].feature1,
        feature2: gameData[i].feature2,
        feature2Value: gameData[i].feature2Value,
        answer: value[i].ig,
      };
    }
  ...
  return (
    <div>
      <Helmet>
        <title>GradedEvaluateIgExpression</title>
        <meta
          name="description"
          content="Description of GradedEvaluateIgExpression"
        />
      </Helmet>
...
GradedEvaluateIgExpression.propTypes = {
...
const mapStateToProps = createStructuredSelector({
  state: makeSelectGradedEvaluateIgExpression(),
});

...
export function* getData() {
  try {
    const response = yield api.get(`/game/find-missing-node/graded-quiz`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/find-missing-node/graded-quiz/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* gradedFindMissingNodeSaga() {

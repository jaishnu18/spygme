...
import GameComponent from 'components/GAMES/DecisionTreeLearning/FindSubsetsOfFeatureComponent';
import { start, end } from 'utils/timerFunctions';
import moment from 'moment';

import { useInjectSaga } from 'utils/injectSaga';
import { useInjectReducer } from 'utils/injectReducer';
import makeSelectFindSubsetsOfFeature from './selectors';
...
export function FindSubsetsOfFeature(props) {
  useInjectReducer({ key: 'findSubsetsOfFeature', reducer });
  useInjectSaga({ key: 'findSubsetsOfFeature', saga });
  ...
  const submit = value => {
    ...
    let colors = [];
    for (let i = 1; i <= gameData.dataset.length; i++)
    {
      colors.push(value['row-' + i]);
    }

    gameData.response = {
      dataset: gameData.dataset,
      labels: gameData.labels,
      feature: gameData.feature,
      answer: colors,
    };
  ...
  return (
    <div>
      <Helmet>
        <title>FindSubsetsOfFeature</title>
        <meta
          name="description"
          content="Description of FindSubsetsOfFeature"
        />
      </Helmet>
...
FindSubsetsOfFeature.propTypes = {
...
const mapStateToProps = createStructuredSelector({
  state: makeSelectFindSubsetsOfFeature(),
});

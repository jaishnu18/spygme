...
export function* getTreeWithDataset(action) {
  try {
    const level = action.payload;

    const response = yield api.get(`/game/find-missing-node/question/${level}`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/find-missing-node/question/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export function* putFeedback(action) {
  try {
    ...
    const response = yield api.put(
      `/game/find-missing-node/feedback-save`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* findMissingNodeSaga() {
  // See example in containers/HomePage/saga.js
  yield all([
    takeLatest(GET_GAME_DATA_START, getTreeWithDataset),

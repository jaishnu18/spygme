...
export function* getDataset(action) {
  try {
    ...
    const response = yield api.get(`/game/evaluate-ig-expression/question/${level}`, {
...
export function* evaluateAnswer(action) {
  try {
    ...
    const response = yield api.post(
      `/game/evaluate-ig-expression/question/validate`,
      studentResponse,
      { headers: { Authorization: localStorage._UFT_ } },
    );
...
export default function* evaluateIgExpressionSaga() {

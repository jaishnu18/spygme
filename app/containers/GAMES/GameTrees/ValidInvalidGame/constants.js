/*
 *
 * ValidInvalidGame constants
 *
 */

export const DEFAULT_ACTION = 'app/ValidInvalidGame/DEFAULT_ACTION';

export const GET_GAME_DATA_START = 'app/ValidInvalidGame/GET_GAME_DATA_START';
export const GET_GAME_DATA_SUCCESS =
  'app/ValidInvalidGame/GET_GAME_DATA_SUCCESS';
export const GET_GAME_DATA_FAILURE =
  'app/ValidInvalidGame/GET_GAME_DATA_FAILURE';

export const EVALUATE_RESPONSE_START =
  'app/ValidInvalidGame/EVALUATE_RESPONSE_START';
export const EVALUATE_RESPONSE_SUCCESS =
  'app/ValidInvalidGame/EVALUATE_RESPONSE_SUCCESS';
export const EVALUATE_RESPONSE_FAILURE =
  'app/ValidInvalidGame/EVALUATE_RESPONSE_FAILURE';

export const PUT_FEEDBACK_START = 'app/ValidInvalidGame/PUT_FEEDBACK_START';
export const PUT_FEEDBACK_SUCCESS = 'app/ValidInvalidGame/PUT_FEEDBACK_SUCCESS';
export const PUT_FEEDBACK_FAILURE = 'app/ValidInvalidGame/PUT_FEEDBACK_FAILURE';

export const _START = 'app/ValidInvalidGame/_START';
export const _SUCCESS = 'app/ValidInvalidGame/_SUCCESS';
export const _FAILURE = 'app/ValidInvalidGame/_FAILURE';
